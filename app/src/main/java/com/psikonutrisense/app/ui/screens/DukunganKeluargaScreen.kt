...
@Composable
fun DukunganKeluargaScreen(
    actionState: UiState<String>,
    existing: WellbeingScreening?,
    onNextClick: () -> Unit,
    onSaveClick: (WellbeingScreening) -> Unit
) {
    val context = LocalContext.current
    var emosional by remember(existing) { mutableStateOf(existing?.dukunganEmosional ?: 0) }
    var informasi by remember(existing) { mutableStateOf(existing?.dukunganInformasi ?: 0) }
    var praktis by remember(existing) { mutableStateOf(existing?.dukunganPraktis ?: 0) }
    ...
    Column(
        ...
        Text(
            "Seberapa besar dukungan yang Bunda dapatkan dari keluarga?",
            fontSize = 15.sp,
            fontWeight = FontWeight.Bold,
        ...
        ScaleSelector0to4("Emosional (mendengarkan, memberi semangat)", emosional, { emosional = it })
        Spacer(modifier = Modifier.height(20.dp))
        ScaleSelector0to4("Informasi (saran, pengetahuan)", informasi, { informasi = it })
        Spacer(modifier = Modifier.height(20.dp))
        ScaleSelector0to4("Praktis (membantu pekerjaan rumah/merawat anak)", praktis, { praktis = it })
        ...
        Button(
            onClick = {
                val base = existing ?: WellbeingScreening(motherId = 0)
                onSaveClick(
                    base.copy(
                        dukunganEmosional = emosional,
                        dukunganInformasi = informasi,
                        dukunganPraktis = praktis
                    )
                )
            },
